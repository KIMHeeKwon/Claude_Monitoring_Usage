...
- **모델별 한도 표시**: Fable이나 Opus처럼 모델별 주간 사용량을 보려면 이 항목을 켜야 합니다. 이 값은
  statusline에는 없고 비공식 조회 경로에만 있습니다. Anthropic 약관은 구독 토큰을 다른 도구에서 쓰는 것을
  금지하고 있으므로, 위험을 아시고 켜십시오. 토큰은 읽기만 하며 저장하거나 전송하지 않습니다.
