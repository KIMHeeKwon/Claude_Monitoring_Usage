...
## 2026-09-03 — 화면 다듬기와 실시간 조회 (세션 1 마지막)

**목표**: 사용자 피드백 반영. 디자인 왕복을 없애는 절차 도입.

**결정사항**: D15(끊김 문구 B안) · D16(2a 계기 가운데) · D17(기본 125%) · D18(확인 파일 선제시 절차)
· D19(**실시간 조회 기본 켜짐** — D9의 기본 꺼짐을 뒤집음).

**산출물**
- `tools/make-review.py` + `docs/screen-review.html` — 앱과 같은 코드로 그리는 확인 파일. 설치 없이 브라우저로 검토
- `src-tauri/src/oauth.rs` — 실시간 조회(M1b). 모델별 창은 `limits` 배열의 `weekly_scoped`에서 읽는다
- 마지막 값 보관(`last-usage.json`), 설정 마이그레이션(`v` 필드)

**실측·사고 기록 (2026-09-03)**
- **데스크톱 앱 Code 탭은 statusline을 실행하지 않는다.** 터미널 CLI에서만 갱신된다. 이 때문에 공식 경로만으로는
  값이 거의 갱신되지 않아 실시간 조회가 사실상 필요해졌다 (D19의 근거).
- `uninstall()`이 `status.json`을 지우도록 만든 것이 **실제로 값 유실을 일으켰다**. 사용자가 메뉴를 껐다 켠 뒤
  화면이 비었다. 삭제를 제거하고 마지막 값 보관을 추가했다.
- 실시간 조회 실측: 5시간 6%, 주간 23%, 주간 Fable 42% (2026-09-03 11:05 기준).

**사용자 지적으로 바뀐 작업 방식**
- 화면 변경은 **구현 전에 제안·승인**을 받는다 (프로젝트 CLAUDE.md에 상주 규칙으로 기록).
- 확인은 설치 파일이 아니라 **확인 파일**로 한다 (D18). 설치 왕복이 비효율적이라는 지적.

**현재 진행도**: M0·M1·M1b 완료. M2(GPU) 착수 전. 사용자의 5시간 한도 초기화(2026-09-03 18:00)를 기다린다.

**다음 단계**
1. M2: GPU 사용률·VRAM 읽기 (Windows NVML → PDH 대체, macOS IOKit). 화면 변경 없음
2. M3: macOS 빌드와 배포 안내
3. 동료 시범 배포 (README의 실시간 조회 문단을 함께 전달)
