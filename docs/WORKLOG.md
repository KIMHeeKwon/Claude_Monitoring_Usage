...
## 2026-09-03 — M2(GPU)와 버그 수정, 사용 안내서 (세션 1 종료)

**목표**: GPU 값 읽기, 사용자가 보고한 문제 해결, 동료 배포용 안내서.

**산출물**
- `src-tauri/src/gpu.rs` — GPU 사용률·전용 메모리. 사용률은 **Windows 성능 카운터**(작업 관리자와 같은 계산),
  이름·메모리는 NVIDIA 라이브러리
- `docs/USAGE.md` — 동료에게 그대로 전달할 사용 안내서 (설치·화면 읽는 법·메뉴·문제 해결·실시간 조회 고지)

**고친 버그 (사용자 보고)**
1. **메뉴 이벤트가 두 번 처리됐다.** 트레이와 앱 두 곳에 핸들러를 달아서, 한 번 클릭에 처리가 두 번 일어났다.
   값을 대입하는 항목(레이아웃·테마)은 결과가 같아 정상으로 보였고 **토글만 제자리로 돌아왔다** —
   "실시간 조회가 켜지지 않는다"의 원인. 같은 원인이 "한도 연결을 눌렀더니 값이 사라졌다"도 만들었다
   (uninstall→install이 연달아 실행되며 status.json 삭제). 핸들러를 한 곳으로 줄이고 400ms 중복 억제 추가.
2. **GPU 사용률이 작업 관리자와 달랐다.** NVIDIA 라이브러리의 값은 "커널이 하나라도 돌던 시간의 비율"이라
   부풀려진다. 같은 순간 실측: NVML 28~33% vs 작업 관리자 3.3%. 성능 카운터 방식으로 바꿔 1.4%로 일치.
   부수 효과로 NVIDIA가 아닌 GPU에서도 사용률이 나온다.

**실측 (2026-09-03)**
- GPU: RTX 4090 · 성능 카운터 1.4% · NVML 메모리 1.8/24.0GB
- 앱 CPU 점유 평균 0.03%, 최대 0.13%, 메모리 68MB (24코어 PC, GPU 포함)
- 값 정확도 확인: 계정 실제 44%/31% vs 앱 표시 43%/30% (3분 시차)

**현재 진행도**: Windows판 기능 완료. M0·M1·M1b·M2 끝. 사용자 확인 통과("제대로 작동하는 것 같습니다").

**다음 단계 (2026-09-08 주에 재개, 사용자 토큰 사정으로 연기)**
1. **M3: macOS 빌드** — MacBook에서 작업. IOKit GPU 읽기, `.dmg` 만들기, Gatekeeper 안내
2. 동료 시범 배포 2~3명 ([[USAGE]] 함께 전달)
3. 필요 시: 저장소 공개 여부 결정(SignPath 무료 서명의 전제), 자동 업데이트
